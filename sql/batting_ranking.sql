WITH batting_kpi AS (select p.player_name,bat.season,SUM(bat.runs) as runs,SUM(bat.matches) as matches,p.player_role,
...
select player_name,player_role,season,ROUND((runs/matches),2) AS average,strike_rate,bdry_pct from batting_kpi;
